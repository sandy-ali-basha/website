...
const fetchData = async (params) => {
  ...
  // return data.apiPricingPlanData;
 return [
  {
      "id": 4987,
...
const deleteInvoice = async (id, get) => {
  const response = await axios.delete("/apps/invoice/delete", { data: id });
  const params = get().params;
  await get().fetchData(params);
...
export const useStore = create((set) => ({
  ...
  deleteInvoice: async (id) => {
    const data = await deleteInvoice(id, set.get);
    set((state) => ({
      ...state,
      data: state.data.filter((invoice) => invoice.id !== id),
    }));
  },
}));
